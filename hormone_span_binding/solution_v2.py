...
class Net(nn.Module):
    def __init__(self, vocab, cfg):
        ...
        if cfg['coh']:
            self.P = nn.Parameter(torch.zeros(len(vocab.p2i), len(vocab.p2i)))
    ...
    def forward(self, b):
        ...
        if self.cfg['coh']:
            prob = torch.softmax(Sc, -1)
            Pb = self.P[pid.unsqueeze(2), pid.unsqueeze(1)] * (1 - torch.eye(24))
            Pb = 0.5 * (Pb + Pb.transpose(1, 2))
            e = torch.einsum('bsk,bck->bsc', prob, Pb)
            Sc = Sc + (e.sum(1, keepdim=True) - e)
...
def train_model(E, vocab, cfg, seed):
    ...
    for ep in range(cfg['epochs']):
        ...
        if cfg['swa'] and ep >= cfg['epochs'] - cfg['swa']:
            sd = {k: v.detach().clone().float() for k, v in net.state_dict().items()}
            if avg is None:
                avg = sd
            else:
                for k in avg:
                    avg[k] += (sd[k] - avg[k]) / (n_avg + 1)
            n_avg += 1
    if avg is not None:
